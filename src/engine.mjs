...
export function createTaskEngine({root,owner,getLlm,clock=Date.now,deadlineMs=HARD_MS}) {
  ...
  async function run(id,exec){caller(exec);return exclusive(id,async()=>{
    ...
    while(SAFE.includes(r.status)){
      ...
      const round={attemptId:randomUUID(),status:'ATTEMPT_COMMITTED',visibleText:'',finish:null,usage:null,costMicros:null,holdMicros:price(inputEstimate,r.request.route.maxTokens,rates)};
      r.rounds.push(round);
